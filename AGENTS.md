...
<!-- BEGIN resource_tally v1.0.0 (managed block — regenerated by resource_tally.py install; edits below will be overwritten) -->
...
NOT hand-count). The ledger `dev/resource_tally/data/resource-ledger.jsonl` is append-only,
per-session, concurrency-safe, and stores measurements only. See `dev/resource_tally/README.md`.
Update the tool itself with `python3 dev/resource_tally/resource_tally.py update`.
<!-- END resource_tally -->
