#!/bin/sh
# resource_tally installer — the `curl | sh` bootstrap.
#
...
#   2. fetch the pinned version's tarball and vendor the CODE into dev/resource_tally/
#      (never data/ — that's the committed ledger),
#   3. hand off to the offline installer:  resource_tally.py install
...
# `dev/resource_tally/install.sh` re-fetches + re-installs (an update). To re-wire a repo
...
RT_REF="${RT_REF:-v1.0.0}"                       # tag / branch / sha — pinned by default
RT_DIR="${RT_DIR:-dev/resource_tally}"           # where to vendor, relative to repo root

say()  { printf 'resource_tally: %s\n' "$*" >&2; }
...
[ -f "$tmp/resource_tally.py" ] || die "unexpected archive layout (no resource_tally.py at root of $RT_REPO@$RT_REF)"

# Vendor CODE only. data/ is intentionally excluded so an update never clobbers the
# host repo's ledger.
mkdir -p "$DEST" "$DEST/hooks"
for f in resource_tally.py VERSION README.md install.sh; do
  [ -f "$tmp/$f" ] && cp "$tmp/$f" "$DEST/$f"
done
[ -d "$tmp/hooks" ] && cp -R "$tmp/hooks/." "$DEST/hooks/"
chmod +x "$DEST/resource_tally.py" "$DEST/install.sh" 2>/dev/null || true
[ -f "$DEST/hooks/post-commit" ] && chmod +x "$DEST/hooks/post-commit"

# Offline from here on: wire hooks + AGENTS.md, stamp the version.
python3 "$DEST/resource_tally.py" install --dir "$RT_DIR"

say "done. Review & commit dev/resource_tally + AGENTS.md to share it."
